...
class Bagger:
    ...
    def fit(self, X, y) -> Bagger:
        ...
        for i in range(self.n_estimators):
            ...
            X_now, y_now = self.object_sampler.sample(X, y)

            feas = self.feature_sampler.sample_indices(X_now.shape[1])
            self.indices.append(feas)

            estimator = self.base_estimator(**self.params)
            estimator.fit(X_now[:, feas], y_now)
    ...
    def predict_proba(self, X) -> np.ndarray:
        ...
        for i in range(self.n_estimators):
            ...
            probas.append(estimator.predict_proba(X[:, indices]))

        means = np.mean(probas, axis=0)
        return means
