...
class BaseSampler:
    ...
    def sample_indices(self, n_objects) -> np.ndarray:
        ...
        if self.bootstrap:
            return self.random_state.choice(n_objects, size=int(self.max_samples * n_objects), replace=True)
        else:
            return self.random_state.choice(n_objects, size=int(self.max_samples * n_objects), replace=False)
